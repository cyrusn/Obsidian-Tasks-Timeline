...
function getMeta(tasks) {
  for (i = 0; i < tasks.length; i++) {
    ...
    if (
      ...
      var timelineDate = tasks[i].scheduled || dailyNoteMatch[1]
      var timelineKey = tasks[i].scheduled ? 'scheduled' : 'unplanned'
      if (!dailyTaskMatch) {
        if (
          moment(timelineDate, dailyNoteFormat).format('YYYY-MM-DD') < today
        ) {
          if (forward == true) {
            timelineDates.push(today)
            if (tasks[i].scheduled) {
              happens['scheduled'] = tasks[i].scheduled
            } else {
              happens['unplanned'] = today
            }
            tasks[i].order = taskOrder.indexOf('unplanned')
            ...
            timelineDates.push(
              moment(timelineDate, dailyNoteFormat).format('YYYY-MM-DD')
            )

            happens[timelineKey] = moment(timelineDate, dailyNoteFormat).format(
              'YYYY-MM-DD'
            )
            tasks[i].order = taskOrder.indexOf('unplanned')
          }
